...
const NotificationPanel = ({ isOpen, onClose, onUpdate, theme = "dark" }) => {
  const [notifications, setNotifications] = useState([]);
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  ...
  return (
    <div
      className={`absolute right-0 mt-2 w-72 sm:w-80 border rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-5 duration-200 ${
        isLight
          ? "bg-white border-slate-200 text-slate-900 shadow-slate-200/50"
          : "bg-slate-900/80 backdrop-blur-2xl border-white/10 text-white shadow-black/40"
      }`}
    >
      {/* Header */}
      <div
      ...
      >
        <div className="flex items-center gap-2">
          ...
          />
          <h3
            className={`font-bold text-sm uppercase tracking-wider ${isLight ? "text-slate-800" : "text-white"}`}
          >
            Notifications
          </h3>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={markAllAsRead}
            title="Mark all as read"
            className={`transition-colors ${isLight ? "text-slate-400 hover:text-slate-800" : "text-gray-400 hover:text-white"}`}
          >
            <Check size={16} />
          </button>
          <button
      ...
      {/* List Area */}
      <div className="relative">
        <div className="max-h-[280px] overflow-y-auto overflow-x-hidden scrollbar-hide pb-2">
          {loading ? (
            <div className="p-8 text-center text-gray-500 text-sm">
              Loading...
            </div>
            ...
            <div className="flex flex-col">
              {notifications.map((n) => (
                <div
                  key={n._id}
                  className={`relative p-3.5 flex gap-3 group hover:translate-x-0.5 transition-all duration-300 cursor-pointer border-b last:border-0 ${
                    isLight
                      ? "border-slate-100 hover:bg-slate-50/70"
                      : "border-white/[0.04] hover:bg-white/[0.04] hover:shadow-[inset_0_0_15px_rgba(255,255,255,0.02)]"
                  } ${
                    !n.isRead
                      ? isLight
                        ? "bg-blue-50/30"
                ...
                >
                  {!n.isRead && (
                    <div
                      className={`absolute left-0 top-0 bottom-0 w-[2px] ${isLight ? "bg-blue-600" : "bg-cyan-400"}`}
                    />
                  )}
                  <div className="mt-1">
                    {!n.isRead ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <span
                        className={`text-[10px] whitespace-nowrap mt-0.5 ${isLight ? "text-slate-400" : "text-gray-500"}`}
                      >
                        {formatDistanceToNow(new Date(n.createdAt), {
                          addSuffix: true,
                        })}
                    ...
                    <p
                      className={`text-xs line-clamp-2 mt-0.5 leading-relaxed ${isLight ? "text-slate-600" : "text-gray-400"}`}
                    >
                      {n.message}
                    </p>
                  </div>
                  <button
                    ...
                    className={`opacity-0 group-hover:opacity-100 p-1 transition-all active:scale-90 ${
                      isLight
        ...
        {/* Soft Bottom Fade for Scroll Indicator */}
        {notifications.length > 3 && (
          <div
            className={`absolute bottom-0 left-0 right-0 h-6 pointer-events-none z-10 bg-gradient-to-t ${
              isLight
      ...
      {notifications.length > 0 && (
        <div
          className={`p-3 border-t text-center relative z-20 ${
            isLight
              ? "bg-slate-50 border-slate-100"
